...
'''
rule align_to_gnomad_major:
    input:
        reads1 = PREFIX_PER + '_1.fq',
        idx = expand(PREFIX_MAJOR_IDX + '-gnomad.{IDX_ITEMS}.bt2', IDX_ITEMS = IDX_ITEMS)
    params:
...
rule align_to_onekg_major:
    input:
        reads1 = PREFIX_PER + '_1.fq',
        idx = expand(PREFIX_MAJOR_IDX + '-1kg.{IDX_ITEMS}.bt2', IDX_ITEMS = IDX_ITEMS)
    params:
...
rule align_to_per:
    ...
    output:
        samA = PER_SAM_A,
        samB = PER_SAM_B
        # samA = os.path.join(DIR_FIRST_PASS, 'chr{}-per_hapA.sam'.format(CHROM)),
        # samB = os.path.join(DIR_FIRST_PASS, 'chr{}-per_hapB.sam'.format(CHROM))
    threads: THREADS
    shell:
