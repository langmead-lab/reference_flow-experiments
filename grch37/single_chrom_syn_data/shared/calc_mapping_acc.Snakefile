...
rule calc_refflow_accuracy:
    input:
        gold = PREFIX_PER + '_1.sam',
        var_reads = PREFIX_PER + '.var',
        sam = os.path.join(DIR_SECOND_PASS, '{}-refflow-{}-{}-liftover-sorted.sam'.format(CHROM, ALN_MAPQ_THRSD, POP_DIRNAME))
    output:
        acc_log = os.path.join(DIR_FIRST_PASS, '{}-refflow-{}-{}.acc_log'.format(CHROM, ALN_MAPQ_THRSD, POP_DIRNAME)),
