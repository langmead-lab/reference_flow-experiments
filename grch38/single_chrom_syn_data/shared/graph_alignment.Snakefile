...
rule vg_map:
    ...
    output:
        bam = os.path.join(DIR_FIRST_PASS,
            'chr{}-vg_{}.bam'.format(CHROM, ALLELE_FREQ_FOR_VG)),
        log_map = os.path.join(DIR_VG, '{INDIV}' + '-chr{}-vg_map.time_log'.format(CHROM))
    threads:
        THREADS
    params:
...
rule sort_vg:
    input:
        os.path.join(DIR_FIRST_PASS, 'chr{}-vg_{}.bam'.format(CHROM, ALLELE_FREQ_FOR_VG))
    output:
        os.path.join(DIR_FIRST_PASS, 'chr{}-vg_{}-sorted.sam'.format(CHROM, ALLELE_FREQ_FOR_VG))
    shell:
        'samtools sort -o {output} -O sam {input}'
...
rule calc_vg_accuracy:
    ...
    output:
        acc_log = os.path.join(DIR_FIRST_PASS, 'chr{}-vg_{}.acc_log'.format(CHROM, ALLELE_FREQ_FOR_VG)),
        acc = os.path.join(DIR_RESULTS, '{INDIV}-' + 'chr{}-vg_{}.acc'.format(CHROM, ALLELE_FREQ_FOR_VG))
    run:
...
rule check_vg_accuracy:
    input:
        expand(
            os.path.join(DIR_RESULTS, '{INDIV}-' + 'chr{}-vg_{}.acc'.format(CHROM, ALLELE_FREQ_FOR_VG)),
            INDIV = INDIV),
    output:
