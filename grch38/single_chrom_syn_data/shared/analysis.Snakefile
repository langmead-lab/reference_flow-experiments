rule get_vcf_stats_for_each_individual:
    input:
        os.path.join(DIR_PER, 'chr{}-per.vcf'.format(CHROM))
    output:
        os.path.join(DIR_PER, 'chr{}-per.stats'.format(CHROM))
    shell:
        '{BCFTOOLS} stats {input} > {output}'

rule summarize_indiv_variants:
    input:
        expand(os.path.join(DIR_PER, 'chr{}-per.stats'.format(CHROM)), INDIV = INDIV)
    output:
