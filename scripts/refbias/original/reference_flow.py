...
def main(fn_vcf, fn_sam, fn_fas, fn_output):
    ...
    for i in range(len(het_vcf)):
        ...
        output = str(counter) + '_refbias.txt'
        cigar_whole_genome_sam.main(vcf, sam, fasta, output)#cigar_whole_genome_sam.main(vcf, sam, fasta, output)
            ...
            if count > 0:
                ...
                if i == 0:
                    ...
                    if count-1 < len(het_site): 
                        het_site[count-1].append(int(spl[1]))
                        ref_count[count-1] = ref_count[count-1] + int(spl[3])
                        alt_count[count-1] = alt_count[count-1] + int(spl[4])
                        gap_count[count-1] = gap_count[count-1] + int(spl[5])
                        other_count[count-1] = other_count[count-1] + int(spl[6])
                        num_reads[count-1] = num_reads[count-1] + int(spl[7])
