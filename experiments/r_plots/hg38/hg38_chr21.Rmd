...
sensitivity_rmethod_name <- c("GRC", "Major", "AFR", "AMR", "SAS", "EAS", "EUR", "Matched", "RefFlow (Major)", "vg", "RefFlow (1)", "RefFlow (1000)", "Personalized")
...
filt_sensitivity_twopass <- c("GRC", "Major", "Matched", "RefFlow (Major)", "vg", "RefFlow (1)", "RefFlow (1000)", "Personalized")
...
# if (save == T){
#   ggsave("sim21_mapping_acc.pdf", p_box_sim21_twopass, width = 7, height = 5)
#   # ggsave("sim21_mapping_acc.jpg", p_box_sim21_twopass, width = 7, height = 5)
# }
...
# df_summary_bias <- read.delim("data/allelic_bias/1125_5indiv.bias.tsv", header = T, sep = "\t")
# df_summary_bias <- read.delim("data/allelic_bias/1128_25indiv.bias.tsv", header = T, sep = "\t")
df_summary_bias <- read.delim("data/allelic_bias/0127_25indiv.bias.tsv", header = T, sep = "\t")
...
bias_method_name <- c("grc", "major-1kg", "chr21-AFR-chr21_superpop_AFR_thrds0.5_S0_b1_ld0", "chr21-AMR-chr21_superpop_AMR_thrds0.5_S0_b1_ld0", "chr21-SAS-chr21_superpop_SAS_thrds0.5_S0_b1_ld0", "chr21-EAS-chr21_superpop_EAS_thrds0.5_S0_b1_ld0", "chr21-EUR-chr21_superpop_EUR_thrds0.5_S0_b1_ld0", "Matched",  "chr21-major-10-thrds0.5_S0_b1_ld0-1kg", "chr21-major-10-thrds0_S1_b1_ld0-1kg", "thrds0_S1_b1000_ld1-1kg", "vg_0.01", "per")
bias_rmethod_name <- c("GRC", "Major", "AFR", "AMR", "SAS", "EAS", "EUR", "Matched", "RefFlow (Major)", "RefFlow (1)", "RefFlow (1000)", "vg", "Personalized")
...
bias_med <- df_summary_bias %>% group_by(Reduced) %>% summarise_at(vars(SUM_TAIL), ~ median(., na.rm = TRUE))
...
p_summary_bias <- ggplot(df_summary_bias, aes(x=Reduced, y=SUM_TAIL, color = Super_Population)) + 
  ...
  ylab("Num. biased sites") + xlab("Method") + labs(color="Super Population") +
  theme_bw() +
...
bias_r2a_median_grc <- bias_r2a_med[bias_r2a_med$Reduced =="GRC",]$REF_TO_ALT
bias_r2a_median_per <- bias_r2a_med[bias_r2a_med$Reduced =="Personalized",]$REF_TO_ALT
...
hist_names <- c('GRC', 'Major', 'RefFlow', 'vg', 'Personalized')
p_refbias_sim21_refflow <- plot_contour_hist(hist_targets, hist_names, save, "hist_contour_sim21_refflow") +
  theme(legend.position = "bottom")
p_refbias_sim21_refflow

print ("grc")
get_bias_stats(df_ref)
print ("major")
get_bias_stats(df_maj)
print ("refflow")
get_bias_stats(df_rf)
print ("vg")
get_bias_stats(df_vg)
print ("per")
get_bias_stats(df_per)
```
...
plot_scatter <- function(df, title="", tail){
    ...
    p_biased_ref <- 
      ...
      ylim(ylim) + xlim(xlim) +
      # ylab("") + xlab("") +
      ylab("Read Bias") + xlab("Allelic Bias") +
...
plot_both_tails <- function(fn){
    ...
    p_biased_rf_ref <- plot_scatter(df_biased_rf, "RefFlow", "REF") + ylab("")
    p_biased_rf_alt <- plot_scatter(df_biased_rf, "RefFlow", "ALT") + ylab("")
...
c_method <- c(rep("GRC", num_class), rep("Major", num_class), rep("RefFlow", num_class), rep("Personalized", num_class))
c_type <- rep(c("Total", "HapDepleted", "HighQual", "LowQual"), num_exp)
c_value <- c(unsuper_cnt_ref, unsuper_cnt_major, unsuper_cnt_rf, unsuper_cnt_per)
dat_unsuper <- data.frame(c_method, c_type, c_value)

level_method <- c("GRC", "Major", "RefFlow", "Personalized")
dat_unsuper$c_method <- factor(dat_unsuper$c_method, levels = level_method)
dat_unsuper$c_type <- factor(dat_unsuper$c_type, levels = c("Total", "HapDepleted", "HighQual", "LowQual"))
...
fig_unsuper_trend <- ggplot(dat_unsuper, aes(fill=c_method, y=c_value, x=c_type)) + 
  ...
  ylab("Num Biased Sites") + xlab("Method") + labs(fill="Group", title="Synthetic-agnostic")
fig_unsuper_trend
...
c_method <- c(rep("GRC", num_class), rep("Major", num_class), rep("RefFlow", num_class), rep("Personalized", num_class))
# c_type <- rep(c("Total", "HapDepleted", "HighQual", "LowQual"), num_exp)
c_type <- rep(c("Total", "Diagonal", "HighQual", "LowQual"), num_exp)
...
dat_super$c_type <- factor(dat_super$c_type, levels = c("Total", "Diagonal", "HighQual", "LowQual"))
...
fig_super_trend <- ggplot(dat_super, aes(fill=c_method, y=c_value, x=c_type)) + 
  ...
  ylab("Num Biased Sites") + xlab("Method") + labs(fill="Group", title="Synthetic-aware")
fig_super_trend
...
compare_group <- function(df){
  ...
  c(total, diag, highq, lowq)
}
# c("total", "diag", "highq", "lowq")
# compare_group(df_biased_ref)
# compare_group(df_biased_maj)
# compare_group(df_biased_rf)
# compare_group(df_biased_per)
...
# ref_alt_frac_ref <- sum(df_ref_real$REF_COUNT) / sum(df_ref_real$ALT_COUNT)
# ref_alt_frac_maj <- sum(df_maj_real$REF_COUNT) / sum(df_maj_real$ALT_COUNT)
# ref_alt_frac_vg <- sum(df_vg_real$REF_COUNT) / sum(df_vg_real$ALT_COUNT)
# ref_alt_frac_rf <- sum(df_rf_real$REF_COUNT) / sum(df_rf_real$ALT_COUNT)
# ref_alt_frac_per <- sum(df_per_real$REF_COUNT) / sum(df_per_real$ALT_COUNT)
...
hist_step <- 0.04
half_step <- hist_step / 2
hist_ref_real <- hist(df_ref_real$REFBIAS, breaks=seq(-half_step, 1+half_step, by=hist_step))$counts
hist_ref_real <- c(hist_ref_real, hist_ref_real[length(hist_ref_real)])
hist_maj_real <- hist(df_maj_real$REFBIAS, breaks=seq(-half_step, 1+half_step, by=hist_step))$counts
hist_maj_real <- c(hist_maj_real, hist_maj_real[length(hist_maj_real)])
hist_rf_real <- hist(df_rf_real$REFBIAS, breaks=seq(-half_step, 1+half_step, by=hist_step))$counts
hist_rf_real <- c(hist_rf_real, hist_rf_real[length(hist_rf_real)])
hist_vg_real <- hist(df_vg_real$REFBIAS, breaks=seq(-half_step, 1+half_step, by=hist_step))$counts
hist_vg_real <- c(hist_vg_real, hist_vg_real[length(hist_vg_real)])
hist_per_real <- hist(df_per_real$REFBIAS, breaks=seq(-half_step, 1+half_step, by=hist_step))$counts
hist_per_real <- c(hist_per_real, hist_per_real[length(hist_per_real)])
...
plot_contour_hist <- function (hist_targets, hist_names, save, prefix){
  ...
  p_hist <- ggplot() + 
    ...
    # scale_y_continuous(limits=c(0,30)) +
    # scale_y_continuous(limits=c(0,50)) + 
    scale_y_continuous(trans = 'log10', limits=c(700,NA)) +
    theme_bw() + 
    xlab("Bias") + ylab("Count") + labs(color='Method') + 
    geom_step(mapping=aes(x=hist_bins, y=df_hist_onepass$xx, colour=df_hist_onepass$yy), size=1) +
...
hist_names_real <- c('GRC', 'Major', 'RefFlow', 'vg', 'Personalized')
p_refbias_real <- plot_contour_hist(hist_targets_real, hist_names_real, save, "hist_wg")
p_refbias_real
...
# fig_unsuper_trend_wg <- ggplot(dat_unsuper, aes(fill=c_type, y=c_value, x=c_method)) +
fig_unsuper_trend_wg <- ggplot(dat_unsuper, aes(fill=c_method, y=c_value, x=c_type)) +
...
simplified_variant_lvl <- c(
  "Major",
  "Major_AFR", "Major_AMR", "Major_EUR", "Major_EAS", "Major_SAS",
...
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AFR_thrds0.5_S0_b1_ld0" = "Major_AFR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AMR_thrds0.5_S0_b1_ld0" = "Major_AMR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EUR_thrds0.5_S0_b1_ld0" = "Major_EUR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EAS_thrds0.5_S0_b1_ld0" = "Major_EAS")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_SAS_thrds0.5_S0_b1_ld0" = "Major_SAS")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AFR_thrds0_S1_b1_ld0" = "Stoc_no_phase_AFR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AMR_thrds0_S1_b1_ld0" = "Stoc_no_phase_AMR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EUR_thrds0_S1_b1_ld0" = "Stoc_no_phase_EUR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EAS_thrds0_S1_b1_ld0" = "Stoc_no_phase_EAS")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_SAS_thrds0_S1_b1_ld0" = "Stoc_no_phase_SAS")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AFR_thrds0_S1_b1000_ld1" = "Stoc_phased_AFR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_AMR_thrds0_S1_b1000_ld1" = "Stoc_phased_AMR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EUR_thrds0_S1_b1000_ld1" = "Stoc_phased_EUR")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_EAS_thrds0_S1_b1000_ld1" = "Stoc_phased_EAS")
df_var_jaccard$x <- recode(df_var_jaccard$x, "chr21_superpop_SAS_thrds0_S1_b1000_ld1" = "Stoc_phased_SAS")

df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21-major-1kg" = "Major")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AFR_thrds0.5_S0_b1_ld0" = "Major_AFR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AMR_thrds0.5_S0_b1_ld0" = "Major_AMR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EUR_thrds0.5_S0_b1_ld0" = "Major_EUR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EAS_thrds0.5_S0_b1_ld0" = "Major_EAS")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_SAS_thrds0.5_S0_b1_ld0" = "Major_SAS")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AFR_thrds0_S1_b1_ld0" = "Stoc_no_phase_AFR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AMR_thrds0_S1_b1_ld0" = "Stoc_no_phase_AMR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EUR_thrds0_S1_b1_ld0" = "Stoc_no_phase_EUR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EAS_thrds0_S1_b1_ld0" = "Stoc_no_phase_EAS")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_SAS_thrds0_S1_b1_ld0" = "Stoc_no_phase_SAS")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AFR_thrds0_S1_b1000_ld1" = "Stoc_phased_AFR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_AMR_thrds0_S1_b1000_ld1" = "Stoc_phased_AMR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EUR_thrds0_S1_b1000_ld1" = "Stoc_phased_EUR")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_EAS_thrds0_S1_b1000_ld1" = "Stoc_phased_EAS")
df_var_jaccard$y <- recode(df_var_jaccard$y, "chr21_superpop_SAS_thrds0_S1_b1000_ld1" = "Stoc_phased_SAS")
...
fig_var_dist_chr21_no_indiv <- ggplot(df_var_jaccard_no_indiv, aes(x, y)) +
  geom_tile(aes(fill = distance), color='white') +
...
df_var_jaccard_major_pop_only <- df_var_jaccard_no_indiv %>% filter (x %in% c("Major_AFR", "Major_AMR", "Major_EUR", "Major_EAS", "Major_SAS") & y %in% c("Major_AFR", "Major_AMR", "Major_EUR", "Major_EAS", "Major_SAS") & x != y)
